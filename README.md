...
A launch detection algorithm takes the x and y position of a click and loops through all the balls to find the closest ball that is within the click area. To accomplish this, it converts the position of each ball from world coordinates to screen coordinates using the view and projection matrices in order to test for 2D intersection.

To calculate the launch velocity of the ball being interacted with by the user, I start with the change in mouse coordinates vec2(-dx, dy). Since the user is able to move around the table, I also rotate this vector by the number of radians the user has covered so that it points in the desired launch direction.

A collision detection algorithm calculates the overlap distance between each pair of two balls, and if the overlap is greater than 0.1, the two balls are moved away from each other by half the overlap distance along the normal vector between them. It then calculates the velocities of the two balls and updates them according to the relative velocities and the collision normal. If one ball is moving faster than the other, it bounces off the other ball while the slower ball is pushed away. If the balls have the same speed, they both bounce off each other. 

A boundary detection algorithm calculates the threshold distance from the edge of the table and checks whether the ball has gone beyond it in either the x or y direction. If the ball is beyond the threshold, the algorithm adjusts its position so that it is within the boundaries of the table and reflects its velocity in the appropriate direction. 
...
The game enables the user to estimate the trajectory of the ball being launched by drawing trajectory dots along the path given by the launch velocity of the ball. The trajectory dots are spaced further apart the further they are from the ball.
...
The orientation of the "Eye of Sauron" is determined by the game state and modified using point billboarding. If the game is just starting, if the ball is not being launched or is moving slowly, or if the game has ended, the eye is pointed at the user. Otherwise, the eye is pointed at the ball most recently interacted with. To calculate the eye orientation, the program first calculates a vector pointing from the camera to the designated look-at position. This vector is then used to calculate two additional vectors using cross product, one perpendicular to the up vector and the look-at vector, and the other perpendicular to both of the first two vectors. These three vectors are then used to create a rotation matrix, which is applied to the eye. The logo is also oriented in this way, but unlike the eye, it is set to face the user at all times.
